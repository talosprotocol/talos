...
export PATH="/Applications/Docker.app/Contents/Resources/bin:$HOME/.cargo/bin:$HOME/.local/bin:$HOME/.pyenv/shims:$PATH"
...
export PYTHONPATH="$ROOT_DIR:$ROOT_DIR/contracts/python:$ROOT_DIR/sdks/python/src"
for d in $(find "$ROOT_DIR/services" "$ROOT_DIR/libs" -type d -name "src" 2>/dev/null); do
...
# 1.3 Baseline PYTHONPATH
export PYTHONPATH="$ROOT_DIR:$ROOT_DIR/contracts/python:$ROOT_DIR/sdks/python/src"

for req in $(find services -name "requirements.txt" 2>/dev/null); do
  dir=$(dirname "$req")
  echo "Installing requirements for $dir..."
  (cd "$dir" && uv pip install -r requirements.txt || true)
done
...
uv run python3 "$SCRIPT_DIR/seed_test_env.py"

# 5. Start Sub-Services in Background
echo "Starting Sub-Services..."

# Kill any lingering uv run uvicorn processes
...
PYTHONPATH="src" \
uv run --project "$ROOT_DIR" python3 -m uvicorn src.adapters.http.main:app --port 8002 --host 127.0.0.1) > "$LOG_DIR/audit.log" 2>&1 &
...
PORT=8003 \
PYTHONPATH="." \
uv run --project "$ROOT_DIR" python3 -m uvicorn main:app --port 8003 --host 127.0.0.1) > "$LOG_DIR/config.log" 2>&1 &
...
PYTHONPATH="." \
uv run --project "$ROOT_DIR" python3 -m uvicorn main:app --port 8082 --host 127.0.0.1) > "$LOG_DIR/mcp.log" 2>&1 &
...
PYTHONPATH="src" \
uv run --project "$ROOT_DIR" python3 -m uvicorn terminal_adapter.main:app --port 8083 --host 127.0.0.1) > "$LOG_DIR/terminal.log" 2>&1 &
...
PYTHONPATH="src" \
uv run --project "$ROOT_DIR" python3 -m uvicorn src.main:app --port 8090 --host 127.0.0.1) > "$LOG_DIR/chat.log" 2>&1 &
...
PYTHONPATH="src" \
uv run --project "$ROOT_DIR" python3 -m uvicorn src.main:app --port 8200 --host 127.0.0.1) > "$LOG_DIR/aiops.log" 2>&1 &
...
PYTHONPATH="." \
uv run --project "$ROOT_DIR" python3 -m uvicorn app.main:app --port 8001 --host 127.0.0.1 --loop asyncio --workers 1)
